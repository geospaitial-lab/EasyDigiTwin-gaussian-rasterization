...
__device__ float4 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy,
                               float kernel_size, const float* cov3D, const float* viewmatrix, float2* q,
                               float3* normal)
{
    ...
    if (det_0 <= 1e-6){
        q[0] = make_float2(0, 0);
        normal[0] = make_float3(0, 0, 0);
    }
    else{
        const float cov_inv_22_inv = 1 / (det_0 + 1e-6);
        q[0] = {(cov[1][0] * cov[2][1] - cov[1][1] * cov[2][0]) * cov_inv_22_inv,
                (cov[0][1] * cov[2][0] - cov[0][0] * cov[2][1]) * cov_inv_22_inv};
        glm::vec3 ray_space_normal = glm::vec3(-q[0].x, -q[0].y, -1);
        glm::vec3 n = J * ray_space_normal;
        n = glm::normalize(n);
        normal[0] = make_float3(n.x, n.y, n.z);
    }
...
renderCUDA(
	...
	float3* __restrict__ final_T,
	uint32_t* __restrict__ n_contrib,
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	float* __restrict__ out_depth,
    float* __restrict__ out_mean_depth,
	...
	float* __restrict__ out_distortion)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (pix_depth <= near_cutoff)
                continue;

			if (mean_depth == 0 && 1 - T >= 0.5f){
			    mean_depth = pix_depth;
			}

            depth += pix_depth * w;

            normal += collected_normals[j] * w;

            const float m = far_cutoff / (far_cutoff - near_cutoff) * (1 - near_cutoff / pix_depth);
			float A = 1 - T;
			distortion += (m * m * A + D_2 - 2 * m * D_1) * w;
			D_2 += m * m * w;
            D_1 += m * w;
	...
	if (inside)
	{
		final_T[pix_id] = make_float3(T, D_1, D_2);
		n_contrib[pix_id] = last_contributor;
		for (int ch = 0; ch < CHANNELS; ch++)
			out_color[ch * H * W + pix_id] = C[ch] + T * bg_color[ch];
        out_depth[pix_id] = depth;
        out_mean_depth[pix_id] = mean_depth;
        ((float3*)out_normal)[pix_id] = normal;
