	...
	const bool debug)
{
  ...
  torch::Tensor out_mean_depth = torch::full({H, W}, 0.0, float_opts);
  torch::Tensor out_normal = torch::full({H, W, 3}, 0.0, float_opts);
  torch::Tensor out_opacity = torch::full({H, W}, 0.0, float_opts);
  torch::Tensor out_distortion = torch::full({H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
        ...
        out_mean_depth.contiguous().data<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_mean_depth, out_normal, out_opacity, out_distortion,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dqs = torch::zeros({P, 2}, means3D.options());
  ...
  // added Gradients. Need to be summed along first dim after being filled by kernels.
  ...
  torch::Tensor dL_dfocal_x = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dfocal_y = torch::zeros({P, 1}, means3D.options());
  ...
  {  
      ...
      dL_dprojmatrix.contiguous().data_ptr<float>(),
      dL_dcampos.contiguous().data_ptr<float>(),
      dL_dfocal_x.contiguous().data_ptr<float>(),
      dL_dfocal_y.contiguous().data_ptr<float>(),
	  debug);
  }

  torch::Tensor dL_dviewmatrix_sum = dL_dviewmatrix.sum(0);
  torch::Tensor dL_dprojmatrix_sum = dL_dprojmatrix.sum(0);
  torch::Tensor dL_dcampos_sum = dL_dcampos.sum(0);
  torch::Tensor dL_dtan_fovx = -dL_dfocal_x.sum(0) * W / (2.0 * tan_fovx * tan_fovx);
  torch::Tensor dL_dtan_fovy = -dL_dfocal_y.sum(0) * H / (2.0 * tan_fovy * tan_fovy);

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales,
   dL_drotations, dL_dviewmatrix_sum, dL_dprojmatrix_sum, dL_dcampos_sum, dL_dtan_fovx, dL_dtan_fovy);
}
